...
32. Fecha 2026-06-19: Se corrigio distribucion de matriculas en `POST /api/enrollments/distribuir` para que `especialidad_id` solo se actualice cuando llega un valor valido (evita error 500 al mover estudiantes sin especialidad seleccionada). Tambien se ajusto `frontend/assets/js/view-cursos.js` para liberar foco (`blur`) antes de cerrar el modal selector y reducir warning de accesibilidad por `aria-hidden`.
33. Fecha 2026-06-19: Se corrigio error SQL `WARN_DATA_TRUNCATED` en distribucion masiva porque la columna `matriculas.estado` no acepta `MATRICULADO` en este entorno. El endpoint `POST /api/enrollments/distribuir` ahora actualiza estado a `ACTIVO` al mover de paralelo, evitando el 500 en produccion.
